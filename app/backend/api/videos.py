...
def initiate_video_from_brief(
    *,
    ...
    task_queue_google.create_http_task(
        project=settings.GCP_PROJECT_ID, queue=settings.TASK_QUEUE_NAME,
        location=settings.TASK_QUEUE_LOCATION, url=f"{settings.WORKER_URL}/process-pipeline",
        payload=payload, service_account_email=settings.WORKER_SA_EMAIL,
    )
